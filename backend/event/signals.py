...
def event_pre_save_signal(sender, instance, **kwargs):
    if instance.event_date <= date_timedelta_1_hour and not instance.in_archive and instance.to_repeat:
        instance.event_date += timezone.timedelta(hours=24)
    elif instance.event_date <= date_timedelta_1_hour and not instance.in_archive:
        instance.in_archive = True


@receiver(post_save, sender=Event)
def event_post_save_signal(sender, instance, created, **kwargs):
    if date_timedelta_1_hour <= instance.event_date <= date_timedelta_2_hours and not instance.in_archive:
